...
def main():
    ...
    # Show trades
    product_id = 'eth-usd'
    df = load('https://api.gdax.com/products/' +
              product_id + '/trades', printout=True)

# Load function

    trade_series = go.Scatter(
        x=df['time'].tolist(),
        y=df['price'].tolist()
    )

    data = [trade_series]
    py.plot(data, filename='trades')
